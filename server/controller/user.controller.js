...
export async function loginController(request, response) {
    try {
        ...
        const refreshToken = await geteratedRefreshToken(user._id)
...
export async function uploadAvatar(request, response) {
    try {
        ...
        const userId = request.userId// auth middeleware
...
export async function verifyForgotPasswordOtp(request, response) {
    try {
        ...
        if (!email || !otp) {
            return response.status(400).json({
                message: "Proivde required field email,otp",
                error: true,
                success: false
            })
        }
...
export async function refeshTokenController(request,response) {
    try{
        ...
        // console.log("veiryToken",verifyToken)
